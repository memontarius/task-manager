<template>
    <Head :title="trans('New task')"/>
    <AuthenticatedLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">{{ $t('New task') }}</h1>
        </template>
        ...
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            <form @submit.prevent="submit">
                ...
                <select
                    ...
                    >
                    <option v-for="status in sortedStatuses" :value="status.id" :key="status.id">{{ status.name }}</option>
                </select>
                ...
                <select
                    ...
                    <option
                        class="bg-none"
                        v-for="label in labels" :value="label.id" :key="label.id">{{ label.name }}</option>
                </select>
